...
// hack
function replaceTreeString(entry){
    if (entry !== undefined) {
        let newTreeString = entry.treeString;
        let length = entry.treeString.length;
        if (newTreeString[length - 2] === "├" && newTreeString[length - 1] === "─") {
            newTreeString = newTreeString.substring(0, newTreeString.length - 2) + "╰─";
            entry.displayName = newTreeString + " " + entry.name;
        }
    }
    return entry;
}
...
function schemaToArray(schema,offset,options,data) {
    ...
    walkSchema(schema,{},wsState,function(schema,parent,state){
        // hack
        if (previousParent !== null && Object.keys(previousParent).length !== 0 && previousParent !== parent
            // special case for type [string] which have a strange parent (and previous parent) and should be ignored, to have a nice tree structure
            && parent?.type !== 'array' && parent?.items?.type !== 'string'
            && previousParent?.type !== 'array' && previousParent?.items?.type !== 'string') {

            // this block is to replace the last treeString of the last entry from ├ to ╰
            let element = block.rows[block.rows.length - 1];
            if (element !== undefined) {
                block.rows.splice(block.rows.length - 1, 1, replaceTreeString(element));
            }
        }
        ...
        if (options.trim && typeof entry.example === 'string') {
            entry.example = entry.example.trim();
        }
        if (options.join && typeof entry.example === 'string') {
            entry.example = entry.example.split('\r').join('').split('\n').join('<br>');
        }
        if (options.truncate && typeof entry.example === 'string') {
            entry.example = entry.example.split('\r').join('').split('\n')[0];
        }
        if (entry.example === 'undefined') { // yes, the string
            entry.example = '';
        }

        if (options.trim && typeof entry.min === 'string') {
            entry.min = entry.min.trim();
        }
        if (options.join && typeof entry.min === 'string') {
            entry.min = entry.min.split('\r').join('').split('\n').join('<br>');
        }
        if (options.truncate && typeof entry.min === 'string') {
            entry.min = entry.min.split('\r').join('').split('\n')[0];
        }
        if (entry.min === 'undefined') { // yes, the string
            entry.min = '';
        }

        if (options.trim && typeof entry.max === 'string') {
            entry.max = entry.max.trim();
        }
        if (options.join && typeof entry.max === 'string') {
            entry.max = entry.max.split('\r').join('').split('\n').join('<br>');
        }
        if (options.truncate && typeof entry.max === 'string') {
            entry.max = entry.max.split('\r').join('').split('\n')[0];
        }
        if (entry.max === 'undefined') { // yes, the string
            entry.max = '';
        }

        if (options.trim && typeof entry.default === 'string') {
            entry.default = entry.default.trim();
        }
        if (options.join && typeof entry.default === 'string') {
            entry.default = entry.default.split('\r').join('').split('\n').join('<br>');
        }
        if (options.truncate && typeof entry.default === 'string') {
            entry.default = entry.default.split('\r').join('').split('\n')[0];
        }
        if (entry.default === 'undefined') { // yes, the string
            entry.default = '';
        }
        ...
        // store in treeString property, a unicode representation of tree structure, to show nested objects
        entry.treeString = "";
        for (let i = 0; i <= entry.depth ; i++){
            if (i == 0 && entry.depth === 1){
                entry.treeString += "├";
            }
            else if (i == 0 && entry.depth !== 1){
                entry.treeString += "│";
            }
            else if (i === entry.depth && (entry.type === 'object' || entry.safeType === '[object]')) {
                // objects and array of objects
                entry.treeString += "┬";
            }
            else if (i === entry.depth && entry.type !== 'object'){
                // simple value
                entry.treeString += "─";
            }
            else if (i == entry.depth - 1) {
                entry.treeString += "├";
            }
            else {
                entry.treeString += "│";
        ...
        entry.displayName = entry.treeString + " " + entry.name
    ...
    // hack:
    // replace last treeString of `block.rows` from "├" to "└"
    let element = block.rows[block.rows.length - 1];
    if (element !== undefined) {
        element.treeString = element.treeString.replace(/├/g, "╰");
        element.treeString = element.treeString.replace(/│/g, "╵");
        element.displayName = element.treeString + " " + element.name;
        block.rows.splice(block.rows.length - 1, 1, element);
    }
    return container;
}
