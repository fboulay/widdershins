...
function getParameters(data) {
    ...
    for (let param of data.parameters) {
        ...
        if (pSchema) {
            ...
            param.example = pSchema.sexample;
            param.min = pSchema.smin;
            param.max = pSchema.smax;
            param.default = pSchema.sdefault;
            if (typeof param.example === 'string') {
              param.example = param.example.trim();
              param.example = param.example.split('\r').join('').split('\n').join('<br>');
            }
            if (param.example === 'undefined') { // yes, the string
              param.example = '';
            }

            if (typeof param.min === 'string') {
              param.min = param.min.trim();
              param.min = param.min.split('\r').join('').split('\n').join('<br>');
            }
            if (param.min === 'undefined') { // yes, the string
              param.min = '';
            }

            if (typeof param.max === 'string') {
              param.max = param.max.trim();
              param.max = param.max.split('\r').join('').split('\n').join('<br>');
            }
            if (param.max === 'undefined') { // yes, the string
              param.max = '';
            }

            if (typeof param.default === 'string') {
              param.default = param.default.trim();
              param.default = param.default.split('\r').join('').split('\n').join('<br>');
            }
            if (param.default === 'undefined') { // yes, the string
              param.default = '';
            }
        }
